...
############################## 1 输入与输出 ##################################
### ==输入== ###
startyear = 2001   ###!!!!请认真修改  , 2001-2022
endyear = 2024
years_length = endyear - startyear + 1

drought_distinguish_way = 3   ### way1/way2:统一干旱事件定义标准为SPEI<-1/-1.4/-1.8   way3:POS-1的那个月SPEI2或SPEI3<10%的阈值
                                ### way3需要配合SPEI2/SPEI3用

spei_length = 2  ###1/2/3

spei_drought_value = -1

spei_drought_threshold_percent = 10   ##!!!!请认真修改  ， 单位是%
spei_wet_threshold_percent = 90       ##!!!!请认真修改  ， 单位是%
...
##################################### 干旱事件识别 Way3： 获取干旱阈值和干旱阈值######################################
### stack all year and month
spei_all = []
for year in range(startyear, endyear+1):

    spei_input_path = rf'D:\NH_SPEI01_03\{year}'

    spei_tiffiles = sorted(glob.glob(os.path.join(spei_input_path, f'*.tif')))
...
for year in range(startyear, endyear+1):
    ...
    ############################### 4.1 获取（SOS）month-（POS）month的SPEI，及干旱事件识别 ####################
            ...
            if spei_length == 1 :
                ...
                if drought_distinguish_way == 4:
                    ...
                    if np.sum(np.isfinite(spei_preseason)) >= 2:
                        # print('Preseason more than 2 months！')
                        current_streak = 0
